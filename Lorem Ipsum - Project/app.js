...
form.addEventListener("submit", function(e){
  //prevent default submitting to server
  e.preventDefault();
  //save the value submitted
  //use parseInt to change type of string to number
  const value = parseInt(amount.value);
  //empty or -1 or greater than 9 set a default action
  if(isNaN(value) || value <= 0 || value > 9){
    //use result to change HTML to include a random paragraph
    //add class result to paragraph for CSS styling
    result.innerHTML = `<p class = "result">${text[randomNumber()]}</p>`
  }else{
    //if in between 0-9 we add the paragraphs in text array
    //use the slice method to get the starting to end (how many paragraphs we want)
    let tempText = text.slice(0, value);
    //use map method to go through array
    //temp text now is an array of the html text.
    tempText = tempText.map(function(item){
      return `<p class = "result">${item}</p>`
      //join sepreates one element of an array from the next
    }).join("");

    //adds html to html document
